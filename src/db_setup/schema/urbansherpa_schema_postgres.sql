create table base_platform.verticals
(
...
comment on table base_platform.verticals is 'Stores verticals'
;

create table base_platform.users
(
...
create unique index base_platform.users_user_name_uindex
	on users (user_name)
;

comment on table base_platform.users is 'Stores user accounts independently of the type of user, this is user accounts for managers, admins, sherpas and customers(for the future)'
;

comment on column base_platform.users.user_name is 'user email to access to the urbansherpa system'
;
